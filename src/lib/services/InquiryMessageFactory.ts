...
export class InquiryMessageFactory {
  createInquiryMessage(store: Store, request: SearchRequest): string {
    const seatingSentence = this.seatingSentenceForPreference(request.seatingPreference);
  ...
  private seatingSentenceForPreference(preference: SearchRequest["seatingPreference"]): string {
    switch (preference) {
      case "together":
...
export const inquiryMessageFactory = new InquiryMessageFactory();
